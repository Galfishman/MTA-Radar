...
for i, (combination, total_score) in enumerate(combinations_with_scores[:num_combinations]):
    ...
    st.write("Players:", ', '.join([f"<span style='color: white;'>{player} ({player_data[player]})</span>" for player in combination]), unsafe_allow_html=True)
    formatted_score = "{:.3f}".format(total_score)
    st.write("Total Score:", f"<span style='color: yellow;'>{formatted_score}</span>", unsafe_allow_html=True)
    st.write("---")
